...
weight: 40
description: >
  Build a real-time view over two different databases — a PostgreSQL orders store and a MySQL vehicles store — joined by license plate, with an embedded Drasi engine, a C# reaction that streams Server-Sent Events, and one integrated web UI that both drives the changes and shows them live.
---
...
This tutorial builds it on **`Drasi`** instead — the .NET library that embeds the Drasi
continuous-query engine directly in your process. A single .NET app connects to *both*
...
reaction**: it shapes each query change in **C#** and streams
...
| **[How It Works](#how)** | Understand the two sources, the cross-database join, the six queries, and the C# reaction that streams SSE | 6 min |
...
- **Ports:** the web UI is on `3000`, PostgreSQL is published on `5842`, and MySQL on `3319`.
  The C# reaction that streams SSE runs inside the app on `8081`, but the app multiplexes it into the UI's
  `/events` stream, so only `3000` needs to be reachable.
{{% /alert %}}

## Step 1 of 4: Set Up Your Environment {#setup}

This tutorial needs **Docker** (it runs PostgreSQL and MySQL) and **.NET 8**. The easiest
...
1. **GitHub Codespaces:** Open
   [this Codespaces link](https://codespaces.new/drasi-project/drasi-dotnet?devcontainer_path=.devcontainer/curbside-pickup/devcontainer.json)
   (it selects **Drasi .NET — Curbside Pickup Tutorial** for you).
...
You'll need **.NET 8**, **Docker** (for PostgreSQL and MySQL), and **bash** (for the
...
`./scripts/start-demo.sh` does two things: it starts **PostgreSQL** (seeded with three orders, all
...
`bootstrap/postgres`, `source/mysql`, `bootstrap/mysql`, and a C# callback reaction) from
`ghcr.io/drasi-project` with `InstallPluginAsync`, which resolves each one to the build that matches your
platform **and** this library version, connects to both databases, bootstraps the existing rows, and
starts the six continuous queries and the C# reaction that streams SSE. When you see this line, it's ready:
...
MySQL's binary log, re-evaluates the affected queries, and the C# reaction that streams SSE re-shapes and pushes
...
docker exec curbside-pickup-dotnet-postgres \
...
docker exec curbside-pickup-dotnet-mysql \
...
Everything you just ran is a single .NET app under `tutorials/curbside-pickup/`. One file,
`Program.cs`, embeds the engine, builds the topology over both databases, wires the SSE
reaction, and serves the UI (the browser front end lives in `wwwroot/`). Here's what each part
does.
...
`pg` is a `JsonObject` of connection settings (host `localhost`, port `5842`, database `RetailOperations`).
...
```csharp
await drasi.AddSourceAsync("mysql", "physical-ops", mysql, bootstrap: Bootstrap("mysql", mysqlBootstrap));
```

`mysql` uses `sslMode: "disabled"` because the tutorial container has no TLS.
...
```csharp
await drasi.AddQueryAsync("delivery", deliveryCypher,
...
reaction — but it's the **same built-in C# reaction that streams SSE** the
[Getting Started tutorial](https://github.com/drasi-project/learning-drasi-server) uses. The
app loads the a C# callback reaction plugin from the OCI registry and adds it with
[`AddReactionAsync`](https://drasi-project.github.io/drasi-dotnet/docs/api/). The reaction opens an
HTTP endpoint and streams each subscribed query's result **changes** to the browser over
Server-Sent Events.

Crucially, the C# reaction shapes each change into a JSON row before sending it
...
Finally, the C# reaction that streams SSE serves each query on its own route. Rather than have the browser open
...
through CDC, re-runs the affected queries, and the C# reaction that streams SSE pushes the shaped change back to
...
- **Sources** connect Drasi to live data — here, **two** of them at once: PostgreSQL via
  logical replication and MySQL via its binary log, both embedded in a single .NET app with
  `Drasi`.
...
From here, try changing the delay threshold, changing the row shape in C#, adding another
cross-source query, or pointing the app at your own data — it's all in one file, `Program.cs`.
