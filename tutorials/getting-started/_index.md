...
weight: 10
description: >
  Your first steps with `Drasi` — embed the Drasi engine in a .NET console app, connect it to a live PostgreSQL message feed, and build five continuous queries (change detection, a filter, an aggregation, a time-based query, and a cross-source join) that print every change to your terminal.
---
...
This tutorial is the best place to start with **`Drasi`**, the .NET library that embeds the
...
queries, and attaches a **C# reaction** — a callback that receives every query-result change
...
**What you'll build:** a running .NET console app that embeds Drasi, connects to PostgreSQL (and a
...
| **[How It Works](#how)** | Understand the sources, the five queries, and the C# reaction | 6 min |
...
- **Ports:** PostgreSQL is published on `5832`, and the app's HTTP source (used in the last step)
  listens on `9100`. There is no web UI.
{{% /alert %}}

## Step 1 of 7: Set Up Your Environment {#setup}

This tutorial needs **Docker** (it runs PostgreSQL) and **.NET 8**. The easiest way to get
...
1. **GitHub Codespaces:** Open
   [this Codespaces link](https://codespaces.new/drasi-project/drasi-dotnet?devcontainer_path=.devcontainer/getting-started/devcontainer.json)
   (it selects **Drasi .NET — Getting Started Tutorial** for you).
...
You'll need **.NET 8** and **Docker** (for PostgreSQL). You drive changes with `docker` and
...
`./scripts/start-demo.sh` starts PostgreSQL (seeded with a `Message` table of four messages) and then runs the
console app in the foreground.

On first start, the app downloads the Drasi plugins it needs — `source/postgres`,
`bootstrap/postgres`, `source/http`, and `bootstrap/scriptfile`. It uses `InstallPluginAsync`, which
...
```bash
docker exec getting-started-dotnet-postgres psql -U drasi_user -d getting_started \
...
```bash
curl -X POST http://localhost:9100/locations \
...
Everything you just ran is a single .NET console app under `tutorials/getting-started/` — and it's
one file. `Program.cs` downloads the plugins, adds the sources, registers the five queries, and
attaches the reaction. Here's what each part does.

### The Sources

The app connects two sources (`Program.cs`). PostgreSQL streams `Message` changes via **logical
replication (CDC)**:

```csharp
await drasi.AddSourceAsync("postgres", "messages", pg, bootstrap: Bootstrap("postgres", pg));
```

`pg` is a `JsonObject` of connection settings (host `localhost`, port `5832`, database `getting_started`, plus `tables`, `slotName`, `publicationName`, and `tableKeys`).
...
with Handlebars templates in the plugin config. It also bootstraps its initial data from a JSONL file via the **ScriptFile**
bootstrap provider**:
...
All five queries are declared explicitly in `Program.cs`, one `AddQuery` call each — with the embedded
...
The join query lists two sources and declares the virtual relationship that connects them via
`QueryOptions.Joins`:
...
```csharp
drasi.AddQuery("messages-with-location", joinCypher,
...
reaction, you attach a **callback** with `AddReaction` and subscribe it to whichever queries you
...
], evt =>
{
    foreach (var diff in evt.Results)
    {
        ...
        {
            case DiffType.Add:
                Console.WriteLine($"  [ADD]    {diff.Data}");
                break;
            case DiffType.Delete:
                Console.WriteLine($"  [DELETE] {diff.Data}");
                break;
            case DiffType.Update:
            case DiffType.Aggregation:
                Console.WriteLine($"  [UPDATE] {diff.Before} -> {diff.After}");
                break;
        }
    }
});
...
That's the whole tutorial — and it fits in a single file. Open
[`Program.cs`](https://github.com/drasi-project/drasi-dotnet/blob/main/tutorials/getting-started/Program.cs)
to read it: the generic host, `AddDrasi`, plugin installs, both sources, the five queries, and the
C# reaction that prints every change.
...
- **Sources** connect Drasi to live data — here, PostgreSQL via Change Data Capture and an HTTP
  source, both embedded directly in a .NET app with `Drasi`.
...
- **Reactions** turn query changes into action. A **C# reaction** (`AddReactionAsync`) handed
...
tutorials, which build full ASP.NET Core applications with a **C# reaction** that streams Server-Sent Events
