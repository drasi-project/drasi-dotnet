...
weight: 20
description: >
  Monitor a smart building's comfort in real time with PostgreSQL CDC, six continuous queries with synthetic joins, and a C# reaction that streams Server-Sent Events to drive a live web UI.
---
...
This tutorial builds a **Building Comfort** monitoring app on **`Drasi`**, the
.NET library that embeds the Drasi continuous-query engine directly in your process.
A PostgreSQL database holds a building made of floors and rooms, each room reporting
`temperature`, `humidity`, and `co2`. A single .NET app connects to the database, runs
...
Instead, the app embeds the engine and adds Drasi's **C# reaction that streams SSE** (`a C# reaction`): it shapes
each query change in **C#** and streams it to the browser over Server-Sent
...
| ---- | ------------- | ---- |
| **[Step 1: Set Up Your Environment](#setup)** | Open the dev container (or install .NET 8 + Docker locally) | 5 min |
...
| **[How It Works](#how)** | Understand the source, the six queries, the synthetic joins, and the C# reaction that streams SSE | 5 min |
...
- **Ports:** the web UI is on `3000` and PostgreSQL is published on `5833`. The C# reaction that streams SSE
...
1. **GitHub Codespaces:** Open
   [this Codespaces link](https://codespaces.new/drasi-project/drasi-dotnet?devcontainer_path=.devcontainer/building-comfort/devcontainer.json)
   (it selects **Drasi .NET — Building Comfort Tutorial** for you).

   Or from the repo page: **Code** → **Codespaces** → **…** next to **Create codespace on
   main** → **New with options…**, set **Dev container configuration** to **Drasi .NET —
   Building Comfort Tutorial**, then create the codespace.

2. **VS Code (local):** Open the
   [`drasi-dotnet`](https://github.com/drasi-project/drasi-dotnet) repository and run
...
You'll need **.NET 8**, **Docker** (for PostgreSQL), and **bash** (for the optional
...
`bootstrap/postgres`, and a C# callback reaction) from `ghcr.io/drasi-project` with `InstallPluginAsync`, which
resolves each one to the build that matches your platform **and** this library version, connects to
the database, and starts the six continuous queries and the C# reaction that streams SSE. When you see this line,
it's ready:
...
affected queries, and the C# reaction that streams SSE re-shapes and pushes the snapshot on its own.
{{% /alert %}}
...
panel is a continuous-query result the C# reaction that streams SSE shaped and pushed.
...
```bash
docker exec building-comfort-dotnet-postgres psql -U drasi_user -d building_comfort \
...
Everything you just ran is a single .NET app under `tutorials/building-comfort/`. One file,
`Program.cs`, embeds the engine, builds the topology, wires the C# reaction that streams SSE, and serves the
UI (the browser front end lives in `wwwroot/`). Here's what each part does.
...
`pg` is a `JsonObject` of connection settings (host `localhost`, port `5833`, database `building_comfort`, plus `tables`, `slotName`, `publicationName`, and `tableKeys`).
...
There are six queries, each registered with its own `AddQueryAsync` call:
...
The C# reaction that streams SSE streams five of these — the per-room feed, the per-floor and building comfort
...
```csharp
var partOfFloor = new QueryJoin
{
    Id = "PART_OF_FLOOR",
    Keys =
    [
        new QueryJoinKey { Label = "Room", Property = "floor_id" },
        new QueryJoinKey { Label = "Floor", Property = "id" },
    ],
};
var partOfBuilding = new QueryJoin
{
    Id = "PART_OF_BUILDING",
    Keys =
    [
        new QueryJoinKey { Label = "Floor", Property = "building_id" },
        new QueryJoinKey { Label = "Building", Property = "id" },
    ],
};

await drasi.AddQueryAsync(id, cypher, ["building-facilities"],
    new QueryOptions { Joins = [partOfFloor, partOfBuilding] });
```
...
### The SSE Reaction (`a C# reaction`)

This is where the Node version differs most from the Drasi Server tutorial's *dashboard*
reaction — but the mechanism is the **same built-in C# reaction that streams SSE** the
[Getting Started tutorial](https://github.com/drasi-project/learning-drasi-server) uses.
The app loads the a C# callback reaction plugin from the OCI registry and adds it with
[`AddReactionAsync`](https://drasi-project.github.io/drasi-dotnet/docs/api/).
...
Crucially, the C# reaction shapes each change into a JSON row before sending
...
```csharp
await drasi.AddReactionAsync("watch", [
...
The reaction copies query columns into the UI's row shape so each value is valid
...
Finally, the C# reaction that streams SSE listens on its own port (`8081`) and serves each query on its own
...
The front end (`wwwroot/`) is a single HTML page with vanilla CSS and JavaScript served from `wwwroot/` — no build
...
affected queries, and the C# reaction that streams SSE pushes the shaped change back to the browser, closing
the loop.
...
- **Sources** connect Drasi to live data — here, PostgreSQL via Change Data Capture,
  embedded directly in a .NET app with `Drasi`.
- **Continuous Queries** with **synthetic joins** let you model relationships
  ...
  automatically.
- **Reactions** turn query changes into action. Drasi's **C# reaction that streams SSE** (`a C# reaction`) shaped
  each change in **C#** and streamed it over **Server-Sent Events** to a
...
From here, try editing the comfort formula, changing the row shape in C#, adding a new
alert query, or pointing the app at your own data — it's all in one file, `Program.cs`.
